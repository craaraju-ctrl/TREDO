//! # NethraBridgeAgent — Python Nethra Proxy via Redis
//!
//! Implements `AgentProvider` by proxying requests to the Python Nethra process
...
//!
//! ```text
//! Rust TREDO                    Redis                       Python Nethra
//! ┌──────────────┐     ┌──────────────┐     ┌──────────────────┐
//! │ AutoTrader   │ ──► │ pub/sub      │ ──► │ nethra_redis_    │
//! │   ↓          │     │ request/     │     │ bridge.py        │
//! │ NethraBridge │ ◄── │ response     │ ◄── │   ↓              │
...
/// AgentProvider that proxies market analysis to Python Nethra via Redis.
///
/// Uses a request/response pattern over Redis pub/sub:
/// 1. Publish a request to `nethra:agent:python_nethra` channel
/// 2. Python Nethra processes and publishes response to `nethra:agent:rust_tredo`
...
/// if Python Nethra is unreachable.
#[derive(Debug)]
pub struct NethraBridgeAgent {
    bridge: Arc<RedisBridge>,
    local_agent_id: String,
    python_agent_id: String,
}

impl NethraBridgeAgent {
    /// Create a new NethraBridgeAgent.
    ///
    ...
    async fn rpc_call(&self, method: &str, params: serde_json::Value) -> Result<serde_json::Value, ProviderError> {
        ...
        for _ in 0..(NETHRA_RPC_TIMEOUT_SECS * 2) {
...
#[async_trait]
impl AgentProvider for NethraBridgeAgent {
    ...
    async fn update_weight(&self, _name: &str, _weight: f64) {
        // Weight updates for Python agents are handled internally by Python Nethra
    }
    ...
    async fn analyze_orchestrated(
        ...
        // Python Nethra handles its own orchestration internally
        None
    }
}
