...
impl KucoinAdapter {
                    ...
                    Ok(resp) => {
                        if let Ok(res) = resp.json::<KucoinSymbolsResponse>().await {
                            if res.code == "200000" {
                                let mut count = 0;
                                for (idx, sym) in
                                    res.data.iter().filter(|s| s.enable_trading).enumerate()
                                {
                                    // Start IDs at offset to prevent overlap with Binance
                                    self.symbol_id_map.insert(
                                        sym.symbol.clone(),
                                        (idx as u16).wrapping_add(5000),
                                    );
                                    count += 1;
                                }
    ...
    fn load_defaults(&self) {
        let defaults = [
            "BTC-USDT", "ETH-USDT", "SOL-USDT", "KCS-USDT", "ADA-USDT", "XRP-USDT",
        ];
        for (idx, sym) in defaults.iter().enumerate() {
            self.symbol_id_map
                .insert(sym.to_string(), (idx as u16).wrapping_add(5000));
        }
        println!(
            "[KucoinAdapter] Loaded {} default fallback symbols.",
            defaults.len()
        );
    }
}
...
pub async fn execute_order(
    ...
    if order_type == "limit" {
        payload["price"] =
            serde_json::json!(price.expect("Limit orders must have a price").to_string());
    ...
    if status.is_success() {
        println!(
            "[KucoinAdapter] Private trade successfully executed: {}",
            body
        );
        Ok(())
    } else {
        println!(
            "[KucoinAdapter] Private trade execution failed. Code: {}, Response: {}",
            status, body
        );
