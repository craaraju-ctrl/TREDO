...
impl TantraService {
    pub fn new() -> Self {
        ...
        tasks.insert(
            ...
            TantraTask {
                ...
                status: "PENDING".to_string(),
                description:
                    "Autonomous agent requests allocation increase from 10% to 15% collateral."
                        .to_string(),
    ...
    pub fn set_dnd(&self, active: bool) {
        ...
        println!(
            "[TantraService] Calendar DND guard status updated to: {}",
            active
        );
    }
    ...
    pub fn resolve_task(&self, task_id: &str) -> bool {
        if let Some(mut task) = self.tasks.get_mut(task_id) {
            ...
            println!(
                "[TantraService] Resolved workflow coworker task: {}",
                task_id
            );
            true
        } else {
