...
//!
//! Provides shared memory between Python Hermes and Rust ARKM agents.
...
pub struct SharedMemoryBlock {
    pub id: String,
    pub namespace: String,        // "python_hermes", "rust_arkm", "shared"
    pub key: String,
...
impl SharedMemory {
    ...
    pub async fn write(&self, namespace: &str, key: &str, value: serde_json::Value) -> Result<SharedMemoryBlock, String> {
        ...
        // Notify Python Hermes via pub/sub
        self.bridge.publish(
            "hermes:memory",
    ...
    pub async fn stats(&self) -> MemoryStats {
        ...
        MemoryStats {
            ...
            python_entries: entries.iter().filter(|e| e.owner == "python_hermes").count(),
            rust_entries: entries.iter().filter(|e| e.owner == "rust_arkm").count(),
