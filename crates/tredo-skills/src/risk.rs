...
impl TradingSkill for PositionSizingSkill {
    ...
    fn description(&self) -> &'static str {
        "Calculates optimal position size based on risk tolerance and account equity"
    }
    ...
    async fn analyze(&self, context: &MarketAnalysisContext) -> Result<SkillSignal, SkillError> {
        if context.cash_available <= 0.0 {
            return Err(SkillError::InvalidParameters(
                "No cash available for position sizing".to_string(),
            ));
        }
        ...
        let position_pct_of_portfolio =
            (recommended_position / context.portfolio_value.max(0.01)) * 100.0;
...
impl TradingSkill for ValueAtRiskSkill {
    ...
    fn description(&self) -> &'static str {
        "Estimates potential loss amount over a given time period at a given confidence level"
    }
    fn category(&self) -> SkillCategory {
        SkillCategory::RiskAssessment
    }
    ...
    async fn analyze(&self, context: &MarketAnalysisContext) -> Result<SkillSignal, SkillError> {
        ...
        if closes.len() < self.lookback_periods + 1 {
            return Err(SkillError::InsufficientData(format!(
                "Need at least {} candles for VaR, got {}",
        ...
        if returns.is_empty() {
            return Err(SkillError::InsufficientData(
                "No returns calculated for VaR".to_string(),
            ));
        }
        ...
        Ok(SkillSignal {
            ...
            details: format!(
                ...
                if daily_var > 0.05 {
                    ...
                    "MODERATE — standard trading risk"
                } else {
                    "LOW — favorable risk environment"
                }
            ),
            indicators,
...
impl TradingSkill for ExposureLimitSkill {
    ...
    fn description(&self) -> &'static str {
        "Ensures position exposure stays within defined risk limits"
    }
    fn category(&self) -> SkillCategory {
        SkillCategory::RiskAssessment
    }
...
impl TradingSkill for VolatilityAnalysisSkill {
    ...
    fn description(&self) -> &'static str {
        "Analyzes price volatility to identify market conditions and adjust strategy"
    }
    fn category(&self) -> SkillCategory {
        SkillCategory::RiskAssessment
    }
    ...
    async fn analyze(&self, context: &MarketAnalysisContext) -> Result<SkillSignal, SkillError> {
        ...
        if closes.len() < self.lookback_periods {
            return Err(SkillError::InsufficientData(format!(
                "Need at least {} candles for volatility analysis, got {}",
        ...
        let true_ranges: Vec<f64> = context
            .candles
            .iter()
        ...
        Ok(SkillSignal {
            ...
            details: format!(
                ...
                if volatility_pct > 5.0 {
                    ...
                    "LOW VOLATILITY — tight ranges, trend trades favorable"
                } else {
                    "MODERATE VOLATILITY — standard market conditions"
                }
            ),
            indicators,
