...
async fn build_test_app_state() -> (
    AppState,
    Arc<Mutex<TradeJournal>>,
    Arc<Mutex<LearningEngine>>,
    Arc<AutoTradingLoop>,
) {
    ...
    let skilled_agent: Arc<dyn AgentProvider> = Arc::new(SkilledAgent::new(nethra.clone()));
    ...
    // ── Learning engine ────────────────────────────────────────────────
    let learning_engine = Arc::new(Mutex::new(LearningEngine::new(LearningConfig::default())));
    ...
    // ── Bridge components ──────────────────────────────────────────────
    let agent_registry = Arc::new(AgentRegistry::new(redis_bridge.clone(), "test_tredo"));
...
async fn test_autotrade_status_returns_initial_state() {
    ...
    let trading = body["trading_state"]
        .as_object()
    ...
    assert_eq!(
        trading["balance"], 100_000.0,
        "Default paper balance should be 100,000"
    );
    assert_eq!(
    ...
    assert!(
        symbols.iter().any(|s| s == "BTC-USD"),
        "Default symbols should include BTC-USD"
    );
    assert!(
        symbols.iter().any(|s| s == "ETH-USD"),
        "Default symbols should include ETH-USD"
    );
}
...
async fn test_autotrade_start_then_stop_toggles_state() {
    ...
    assert!(
        after_start.enabled,
        "Auto-trader should be enabled after start"
    );
    ...
    let body: serde_json::Value = resp.json().await.unwrap();
    assert_eq!(
        body["trading_state"]["enabled"], true,
        "Status endpoint should reflect enabled"
    );
    ...
    assert!(
        !after_stop.enabled,
        "Auto-trader should be disabled after stop"
    );
}
...
async fn test_autotrade_multiple_toggles() {
    ...
    for (expected_enabled, action) in [
        (true, "start"),
        (false, "stop"),
        (true, "start"),
        (false, "stop"),
        ...
        assert_eq!(
            ...
            "Status endpoint should reflect state after '{}'",
            action
        );
    }
}
...
async fn test_autotrade_idempotent_start_stop() {
    ...
    assert!(
        auto_trader.get_state().await.enabled,
        "Should remain enabled after second start"
    );
    ...
    assert!(
        !auto_trader.get_state().await.enabled,
        "Should remain disabled after second stop"
    );
}
...
async fn test_journal_stats_empty_database() {
    ...
    assert_eq!(
        direct_stats.total_trades, 0,
        "Direct DB query should match API"
    );
}
...
async fn test_journal_stats_reflects_persisted_trades() {
    ...
    // ── Step 1: Simulate the auto-trader recording trades ──────────
    {
        ...
        j.close_trade("test-trade-1", 65000.0, "Take profit at resistance")
            .unwrap();
    ...
    assert_eq!(
        ...
        "Only closed trades count in stats"
    );
    assert_eq!(stats["winning_trades"], 1, "BTC trade was profitable");
    assert!(
        stats["total_pnl"].as_f64().unwrap() > 0.0,
        "BTC profit should be positive"
    );
...
async fn test_journal_stats_mixed_trades() {
    ...
    // Win rate = (3/5) * 100 = 60%
    assert!(
        (stats["win_rate"].as_f64().unwrap() - 60.0).abs() < 0.01,
    ...
    assert!(
        (stats["total_pnl"].as_f64().unwrap() - expected_pnl).abs() < 0.01,
        "Total P&L should be ${}",
        expected_pnl
    );
}
    ...
    // Record 3 decisions with different symbols
    {
        ...
        for i in 0..3 {
            let decision = tredo_journal::DecisionRecord {
                ...
                overall_direction: if i % 2 == 0 {
                    "BUY".to_string()
                } else {
                    "SELL".to_string()
                },
                market_regime: "ranging".to_string(),
                action_taken: if i % 2 == 0 {
                    "BUY".to_string()
                } else {
                    "SKIP".to_string()
                },
