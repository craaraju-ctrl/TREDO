...
pub struct IntelligencePool {
    /// Pluggable agent provider (SkilledAgent, HermesBridgeAgent, etc.)
...
impl IntelligencePool {
    ...
    pub async fn query_analyst(&self, prompt: &str) -> Result<String, &'static str> {
        ...
        if let Some(ref tc) = self.tiered_cache {
            ...
            if let Some(cached) = tc.get(&cache_key).await {
                println!("[IntelligencePool] 🎯 L2 Redis cache HIT (shared with Python Hermes)");
    ...
    pub async fn analyze_with_skills(&self, context: MarketAnalysisContext) -> AggregatedAnalysis {
        let agent = self.agent.read().expect("Agent RwLock poisoned").clone();
        agent.analyze_market(&context).await.unwrap_or_else(|_| {
            AggregatedAnalysis {
                symbol: context.symbol.clone(),
                current_price: context.current_price,
                signals: vec![],
                overall_conviction: 0.0,
                overall_direction: arkm_core::SignalDirection::Neutral,
                bullish_signals: 0,
                bearish_signals: 0,
                neutral_signals: 0,
