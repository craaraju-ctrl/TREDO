...
impl TradeJournal {
        ...
        let trade = conn.query_row(
            ...
            |row| {
                Ok(TradeRecord {
                    ...
                    entry_reasoning: row.get(6)?,
                    exit_reasoning: None,
                    market_regime: row.get(7)?,
                    strategies_used: row.get(8)?,
    ...
    pub fn get_performance_stats(&self) -> SqlResult<PerformanceStats> {
        ...
        let (total_pnl, avg_win, avg_loss): (f64, f64, f64) = conn.query_row(
            "SELECT COALESCE(SUM(pnl), 0), COALESCE(AVG(CASE WHEN pnl > 0 THEN pnl END), 0), COALESCE(AVG(CASE WHEN pnl < 0 THEN pnl END), 0)
             FROM trades WHERE is_open = 0 AND pnl IS NOT NULL",
            [],
