...
// ── Re-export shared types from arkm-core ────────────────────────────────
pub use arkm_core::{
...
/// HermesAgent now uses the HermesOrchestrator internally to coordinate
...
pub struct HermesAgent {
    orchestrator: tokio::sync::Mutex<sub_agents::HermesOrchestrator>,
...
impl HermesAgent {
    pub fn new() -> Self {
        ...
        Self {
            orchestrator: tokio::sync::Mutex::new(sub_agents::HermesOrchestrator::new()),
            skills,
        }
    }
    ...
    pub async fn analyze(&self, context: &MarketAnalysisContext) -> AggregatedAnalysis {
        ...
        for skill in &self.skills {
            match skill.analyze(context).await {
                ...
                Err(e) => {
                    eprintln!("[HermesAgent] Skill '{}' error: {}", skill.name(), e);
                }
            }
        }
...
impl std::fmt::Debug for HermesAgent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermesAgent")
