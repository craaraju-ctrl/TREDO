...
impl Channels {
    ...
    pub const GLOBAL: &'static str = "nethra:global";
    /// Agent-to-agent direct messages
    pub fn direct(agent_id: &str) -> String {
        format!("nethra:agent:{}", agent_id)
    }
    /// Sub-agent analysis results
    pub const ANALYSIS: &'static str = "nethra:analysis";
    /// Trade signals from Rust to Python
    pub const TRADE_SIGNALS: &'static str = "nethra:trade_signals";
    /// State sync requests/responses
    pub const STATE_SYNC: &'static str = "nethra:state_sync";
    /// Heartbeat channel
    pub const HEARTBEAT: &'static str = "nethra:heartbeat";
    /// Memory updates (RAG writes)
    pub const MEMORY_UPDATES: &'static str = "nethra:memory";
}
...
impl StateKeys {
    ...
    pub const SHARED_MEMORY: &'static str = "nethra:shared_memory";
    /// KV cache namespace
    pub fn cache_key(key: &str) -> String {
        format!("nethra:cache:{}", key)
    }
    /// RAG embedding store
    pub fn rag_key(key: &str) -> String {
        format!("nethra:rag:{}", key)
    }
    /// Per-agent state
    pub fn agent_state(agent_id: &str) -> String {
        format!("nethra:agent_state:{}", agent_id)
    }
}
...
impl RedisBridge {
        ...
        let heartbeat = tokio::spawn(async move {
            loop {
                ...
                if let Some(conn) = conn_arc.lock().await.as_mut() {
                    let _: Result<(), _> = conn
                        .set_ex::<_, _, ()>(
                            format!("nethra:heartbeat:{}", bridge_id),
    ...
    pub async fn get_connected_agents(&self) -> Result<Vec<String>, String> {
        ...
        let agents: Vec<String> = keys
            .into_iter()
            .map(|k| k.trim_start_matches("nethra:heartbeat:").to_string())
            .collect();
