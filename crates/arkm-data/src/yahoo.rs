...
impl YahooFinanceProvider {
    pub fn new() -> Self {
        let client = reqwest::Client::builder()
            ...
            .user_agent("ARKM-Trading-Terminal/1.0.0 (Mozilla/5.0 compatible)")
            .build()
            .unwrap_or_default();
        Self { client }
    }

    /// Format symbol for Yahoo Finance (e.g., "BTC-USD", "AAPL")
    fn format_symbol(symbol: &str) -> String {
        // Yahoo uses hyphen for crypto pairs, direct ticker for stocks
        let s = symbol.replace('/', "-");
        if s.contains('-') && !s.ends_with("=X") {
